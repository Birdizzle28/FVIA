  ...
  const cardsHtml = data.map(m => {
    ...
    return `
      <article class="card video-card" data-id="${m.id}">
        ...
        <div class="video-tags">${tagBadges}</div>
...
function updateVideoStatusLabels() {
  ...
  Object.entries(all).forEach(([id, v]) => {
    ...
    if (v.completed) el.textContent = 'Completed';
    else if (v.pct) el.textContent = `${Math.round(v.pct)}% watched`;
