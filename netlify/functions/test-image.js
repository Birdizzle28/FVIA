...
function buildOverlaySvg({ width, height }) {
  ...
  <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- MTD -->
    <text x="${width / 2}" y="${mtdY}" font-size="22" font-weight="800" fill="${colorDark}" text-anchor="middle"
          font-family="Arial, sans-serif">
      Month-to-date AP: ${money(mtdAp)}
    </text>

    <!-- Daily -->
    <text x="${width / 2}" y="${dailyY}" font-size="22" font-weight="800" fill="${colorDark}" text-anchor="middle"
          font-family="Arial, sans-serif">
