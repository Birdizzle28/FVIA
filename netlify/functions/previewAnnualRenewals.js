// netlify/functions/previewAnnualRenewals.js
...
if (!supabaseUrl || !serviceKey) {
  console.warn('[previewAnnualRenewals] Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY env vars');
}
...
export async function handler(event) {
  ...
  try {
    ...
    console.log('[previewAnnualRenewals] run_date =', runDateStr);
    ...
    if (priorErr) {
      console.error('[previewAnnualRenewals] Error loading prior renewals:', priorErr);
      return {
        statusCode: 500,
        body: JSON.stringify({
    ...
    if (polErr) {
      console.error('[previewAnnualRenewals] Error loading policies:', polErr);
      return {
        statusCode: 500,
    ...
    if (!policies || policies.length === 0) {
      return {
        ...
        body: JSON.stringify({
          ...
          run_date: runDateStr,
          renewals_created_preview: 0,
          policies_with_renewals_preview: 0,
          total_renewal_gross_preview: 0,
          renewal_rows_preview: [],
        }),
      };
    }
    ...
    async function getAgent(agentId) {
      ...
      if (error || !data) {
        console.warn('[previewAnnualRenewals] Missing agent record for id:', agentId, error);
    ...
    async function getSchedule(policy, level) {
      ...
      if (error || !data) {
        console.warn('[previewAnnualRenewals] No schedule for', key, error);
    ...
    // -------------------------------------------------
    // 4) Build renewal ledger rows (PREVIEW ONLY – NO INSERT)
    // -------------------------------------------------
    const newLedgerRows = [];
    ...
    // *** IMPORTANT DIFFERENCE FROM runAnnualRenewals ***
    // - We DO NOT insert newLedgerRows into commission_ledger.
    // - This function is READ-ONLY. Just a preview.

    return {
      statusCode: 200,
      body: JSON.stringify(
        {
          message: 'Annual renewals PREVIEW completed (no DB changes).',
          run_date: runDateStr,
          renewals_created_preview: newLedgerRows.length,
          policies_with_renewals_preview: policiesWithRenewals,
          total_renewal_gross_preview: round2(totalRenewalGross),
          renewal_rows_preview: newLedgerRows,
        },
        null,
        2
      ),
    ...
    console.error('[previewAnnualRenewals] Unexpected error:', err);
    return {
      statusCode: 500,
