...
export default async () => {
  try {
    ...
    if (apptErr) return json(500, { ok: false, error: apptErr.message });
    ...
    const due = (appts || []).filter((a) => {
      ...
      // 0 mins = at start time (with small grace window)
      const graceMs = 3 * 60 * 1000;
      return now.getTime() >= fireAt.getTime() && now.getTime() <= start.getTime() + graceMs;
    });

    if (due.length === 0) {
      return json(200, { ok: true, sent: 0, message: "Nothing due right now." });
    }
    ...
    return json(200, { ok: true, sent: sentCount });
  } catch (err) {
